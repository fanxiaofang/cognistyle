...
export default function CognitiveHandbook({ onClose }: CognitiveHandbookProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-hidden select-none font-mono">
      ...
      <motion.div
        ...
        className="relative w-full max-w-6xl h-[90vh] bg-[#050814] border-2 border-[#00f0ff]/80 shadow-[6px_6px_0px_rgba(255,0,127,0.6)] flex flex-col overflow-hidden"
      >
        {/* Terminal Header */}
        <div className="flex justify-between items-center bg-[#070b19] border-b-2 border-dashed border-[#00f0ff]/40 px-5 py-4">
          <div className="flex items-center gap-3">
            <BookOpen className="w-5 h-5 text-[#00f0ff]" />
            <div>
              {/* <span className="text-[9px] font-pixel text-[#ffe600] uppercase block tracking-wider mb-0.5 leading-none">
                COGNISTYLE MASTER ARCHIVES
              </span> */}
              <h2 className="text-base sm:text-lg font-black text-white tracking-widest font-display leading-none">
                认知风格全图鉴
              </h2>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 border-2 border-[#ff007f] hover:bg-[#ff007f] hover:text-white text-[#ff007f] bg-black transition-colors pointer-events-auto cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {GUILDS.map(g => {
            ...
            return (
              <button
                ...
                className={`py-3.5 px-3 flex flex-col items-center justify-center gap-1 border-r border-[#00f0ff]/10 text-center transition-all cursor-pointer relative
                  ${isActive 
              ...
              >
                <div className="flex items-center gap-1.5">
                  <Icon className={`w-4 h-4 ${isActive ? g.color : 'text-slate-500'}`} />
                  <span className={`text-xs ${isActive ? g.glowClass : ''} tracking-wide text-[13px] sm:text-xs`}>
                    {g.name.split(' (')[0]}
                  </span>
                </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#050814]/70">
          
          {/* Left: Interactive List of Cards */}
          <div className="w-full md:w-[45%] lg:w-[40%] p-4 border-b md:border-b-0 md:border-r border-[#00f0ff]/20 overflow-y-auto flex flex-col gap-4 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-slate-950">
            {/* Guild Intro */}
            <div className={`p-4 bg-black border-2 border-dashed ${selectedGuild.borderColor} rounded-none`}>
              <h3 className={`text-sm font-black flex items-center gap-2 mb-1.5 ${selectedGuild.color} uppercase tracking-widest`}>
              ...
              {selectedGuild.members.map(member => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 border-2 text-left transition-all cursor-pointer select-none rounded-none
                      ${isSelected 
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`p-1 bg-black border-2 shrink-0 ${isSelected ? selectedGuild.borderColor : 'border-slate-800'}`}>
                        <PixelAvatar id={member.profileId} size={48} glow={isSelected} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-1">
                          <span className="text-[13px] font-pixel text-[#00f0ff] glow-cyan">{member.profileId}</span>
                        </div>
                        <h4 className="text-sm font-black text-white leading-tight font-display tracking-widest">
                          {profile.displayName}
                        </h4>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[13px] font-pixel text-[#ffe600]">{member.callSign}</span>
                          <span className="text-[13px] text-slate-500">|</span>
                          <span className="text-[13px] text-slate-400 font-sans">{member.department}</span> 
                          <span className="text-[13px] text-slate-600">·</span>
                          <span className="text-[13px] text-slate-500 font-sans">{member.role}</span>
                        </div>
                        <p className="text-[13px] text-slate-500 font-sans truncate tracking-tight mt-1">{profile.flavorText}</p>
                      </div>
                    </div>
                  </button>
                );
          ...
          <div className="flex-1 p-5 md:p-8 overflow-y-auto bg-black border-t md:border-t-0 md:border-l border-[#00f0ff]/10 relative flex flex-col scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-slate-950">
                ...
                (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-5 pb-5 border-b border-dashed border-[#00f0ff]/30">
                        <div className={`p-1.5 bg-black border-2 ${selectedGuild.borderColor} shadow-[3px_3px_0px_rgba(5,8,20,0.6)]`}>
                          <PixelAvatar id={selectedProfileId} size={72} />
                        </div>
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-[13px] font-pixel text-[#00f0ff] glow-cyan">
                              COGNISTYLE CODE: {selectedProfileId}
                            </span>
                            {member && (
                              <span className={`text-[13px] font-pixel px-1.5 py-0.5 border ${selectedGuild.borderColor} text-slate-300`}>
                                {member.role}
                              </span>
                            )}
                          </div>
                          <h3 className="text-xl sm:text-2xl font-black text-white py-1 font-display tracking-widest uppercase">
                            {profile.displayName}
                          </h3>
                          <div className="flex items-center gap-3 flex-wrap">
                            <span className="text-[13px] text-[#ff007f] font-mono tracking-wide italic font-bold">
                              「 {profile.flavorText} 」
                            </span>
                            {member && (
                              <span className="text-[13px] font-pixel text-[#ffe600] bg-black border border-[#ffe600]/30 px-1.5 py-0.5">
                      ...
                      <div className={`-mx-2 px-4 py-3 bg-black border-l-4 ${selectedGuild.borderColor}`}>
                      ...
                      <div className="space-y-2">
                        <span className="text-[13px] font-pixel text-slate-500 uppercase tracking-widest block">    
                          [ SYSTEM DECODER REPORT ]
                        </span>
                        <p className="text-[13px] sm:text-sm text-slate-300 leading-relaxed bg-[#070b19] p-5 border border-dashed border-slate-800 font-sans shadow-inner">
                          {profile.essence}
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                        {/* Strengths */}
                        <div className="bg-black border-2 border-[#122e23] p-4">
                          <h4 className="text-[13px] font-pixel text-[#39ff14] flex items-center gap-1.5 mb-3 uppercase tracking-wider glow-green">
                            <CheckCircle2 className="w-4 h-4 text-[#39ff14]" />
                            核心优势
                          </h4>
                          <ul className="space-y-1.5 font-sans mt-3">
                            {profile.workplaceEdge.filter(e => e.startsWith('优势：')).map((st, i) => (
                              <li key={i} className="text-[13px] leading-snug text-slate-400 flex items-start gap-1.5">
                                <CornerDownRight className="w-2.5 h-2.5 text-[#39ff14] shrink-0 mt-0.5" />
                        ...
                        {/* Weaknesses */}
                        <div className="bg-black border-2 border-[#2f2711] p-4">
                          <h4 className="text-[13px] font-pixel text-[#ffe600] flex items-center gap-1.5 mb-3 uppercase tracking-wider glow-yellow font-bold">
                            <AlertTriangle className="w-4 h-4 text-[#ffe600]" />
                            潜在盲区
                          </h4>
                          <ul className="space-y-2 font-sans">
                            {profile.workplaceEdge.filter(e => e.startsWith('边界：')).map((we, i) => (
                              <li key={i} className="text-[13px] sm:text-xs leading-relaxed text-slate-300 flex items-start gap-2">
                      ...
                      {/* Cognitive Pattern */}
                      <div className="border-t border-dashed border-[#00f0ff]/30 pt-5">
                        <h4 className="text-[13px] font-pixel text-[#00f0ff] flex items-center gap-1.5 mb-3 uppercase tracking-wider glow-cyan">
                          <Sparkles className="w-4 h-4 text-[#00f0ff]" />
                          认知模式
                        </h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {profile.cognitivePattern.map((cp, i) => (
                            <div key={i} className="bg-[#ff007f]/5 border border-[#ff007f]/20 p-3">
                              <p className="text-[13px] leading-relaxed text-slate-300 font-medium">{cp}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      {/* Collaboration */}
                      <div className="border-t border-dashed border-[#ffe600]/30 pt-5">
                        <h4 className="text-[13px] font-pixel text-[#ffe600] flex items-center gap-1.5 mb-3 uppercase tracking-wider glow-yellow font-bold">
                          <CornerDownRight className="w-4 h-4 text-[#ffe600]" />
                          协作风格
                        </h4>
                        <p className="text-[13px] leading-relaxed text-slate-400 bg-[#070b19] p-4 border border-dashed border-[#ffe600]/30 font-sans">
                          {profile.collaboration}
                        </p>
                      </div>

                      {/* Growth Tip */}
                      <div className="border-t border-dashed border-[#39ff14]/30 pt-5">
                        <h4 className="text-[13px] font-pixel text-[#39ff14] flex items-center gap-1.5 mb-3 uppercase tracking-wider glow-green">
                          <CornerDownRight className="w-4 h-4 text-[#39ff14]" />
                          成长建议
                        </h4>
                        <p className="text-[13px] leading-relaxed text-slate-400 bg-[#070b19] p-4 border border-dashed border-[#39ff14]/30 font-sans">
                          {profile.growthTip}
                        </p>
                      </div>

                      {/* Visual Theme */}
                      <div className="border-t border-dashed border-[#00f0ff]/20 pt-5">
                        <h4 className="text-[13px] font-pixel text-[#00f0ff] flex items-center gap-1.5 mb-3 uppercase tracking-wider glow-cyan">  
                          <Sparkles className="w-4 h-4 text-[#00f0ff]" />
                          视觉场景
                        </h4>
